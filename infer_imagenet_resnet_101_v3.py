...
def construct_batch_normalisation_block(current, net, weights, start_weight_index, param_names, layer_names, is_training):
    ...
    def mean_var_with_update():
        updated_mean = mean * decay + batch_mean * (1 - decay)
        updated_variance = variance * decay + batch_variance * (1 - decay)
        return updated_mean, updated_variance

    mean, variance = tf.cond(pred=is_training, true_fn=mean_var_with_update, false_fn=lambda: (mean, variance))
