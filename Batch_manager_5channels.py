    ...
    def next_batch(self, saver, batch_size, input_tensor, logits, keep_probability, sess, log_dir, is_validation=False):
        ...
        if self.batch_offset > self.images.shape[0]:
            ...
            if not is_validation:
                eval_dir(input_tensor, logits, keep_probability, sess, batch_size, log_dir, self.epochs_completed, is_validation=False, num_channels=5)
                eval_dir(input_tensor, logits, keep_probability, sess, batch_size, log_dir, self.epochs_completed, is_validation=True, num_channels=5)
            # Shuffle the data
