...
from build_resnet_graph_15channels import resnet101_net
...
def inference(image, keep_prob, is_training):
    ...
    mean_pixel[:, :, 3] = np.ones((IMAGE_SIZE, IMAGE_SIZE)) * 30.69861307993539 # nDSM
    mean_pixel[:, :, 4] = np.ones((IMAGE_SIZE, IMAGE_SIZE)) * 284.9702          # DSM
    mean_pixel[:, :, 5] = np.ones((IMAGE_SIZE, IMAGE_SIZE)) * 136.495572072     # A
    mean_pixel[:, :, 6] = np.ones((IMAGE_SIZE, IMAGE_SIZE)) * -0.0827414        # azi
    mean_pixel[:, :, 7] = np.ones((IMAGE_SIZE, IMAGE_SIZE)) * 106.472206683     # B
    mean_pixel[:, :, 8] = np.ones((IMAGE_SIZE, IMAGE_SIZE)) * 1.12959           # ele
    mean_pixel[:, :, 9] = np.ones((IMAGE_SIZE, IMAGE_SIZE)) * 1.74663508206     # entpy
    mean_pixel[:, :, 10] = np.ones((IMAGE_SIZE, IMAGE_SIZE)) * 2.01737815343    # entpy2
    mean_pixel[:, :, 11] = np.ones((IMAGE_SIZE, IMAGE_SIZE)) * 91.7477946018    # L
    mean_pixel[:, :, 12] = np.ones((IMAGE_SIZE, IMAGE_SIZE)) * 4.82043402578    # ndvi
    mean_pixel[:, :, 13] = np.ones((IMAGE_SIZE, IMAGE_SIZE)) * 79.7208191052    # sat
    mean_pixel[:, :, 14] = np.ones((IMAGE_SIZE, IMAGE_SIZE)) * 25.6281567428    # texton
...
def main(argv=None):
    ...
    train_records, valid_records = reader.read_dataset_15_test(FLAGS.data_dir)
    ...
    if FLAGS.mode == "train":
        for itr in xrange(MAX_ITERATION):
            train_images, train_annotations = train_dataset_reader.next_batch_15(saver, FLAGS.batch_size, image, logits, keep_probability, sess, is_training, FLAGS.logs_dir)
            ...
            if itr % 600 == 0:
                valid_images, valid_annotations = validation_dataset_reader.next_batch_15(saver, FLAGS.batch_size, image, logits, keep_probability, sess, is_training, FLAGS.logs_dir, is_validation=True)
