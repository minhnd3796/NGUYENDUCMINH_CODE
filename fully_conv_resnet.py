...
def inference(image, keep_prob):
    ...
    with tf.variable_scope("inference"):
        ...
        deconv_shape1 = image_net["res4b22_relu"].get_shape()
        W_t1 = utils.weight_variable([4, 4, deconv_shape1[3].value, NUM_OF_CLASSESS], name="W_t1")
        b_t1 = utils.bias_variable([deconv_shape1[3].value], name="b_t1")
        conv_t1 = utils.conv2d_transpose_strided(conv8, W_t1, b_t1, output_shape=tf.shape(image_net["res4b22_relu"]))
        fuse_1 = tf.add(conv_t1, image_net["res4b22_relu"], name="fuse_1")

        deconv_shape2 = image_net["res3b3_relu"].get_shape()
        W_t2 = utils.weight_variable([4, 4, deconv_shape2[3].value, deconv_shape1[3].value], name="W_t2")
        b_t2 = utils.bias_variable([deconv_shape2[3].value], name="b_t2")
        conv_t2 = utils.conv2d_transpose_strided(fuse_1, W_t2, b_t2, output_shape=tf.shape(image_net["res3b3_relu"]))
        fuse_2 = tf.add(conv_t2, image_net["res3b3_relu"], name="fuse_2")

        deconv_shape3 = image_net["res2c_relu"].get_shape()
        W_t3 = utils.weight_variable([4, 4, deconv_shape2[3].value, deconv_shape2[3].value], name="W_t3")
        b_t3 = utils.bias_variable([deconv_shape2[3].value], name="b_t3")
        conv_t3 = utils.conv2d_transpose_strided(fuse_2, W_t3, b_t3, output_shape=tf.shape(image_net["res2c_relu"]))
        fuse_3 = tf.add(conv_t3, image_net["res2c_relu"], name="fuse_3")

        shape = tf.shape(image)
        deconv_shape4 = tf.stack([shape[0], shape[1], shape[2], NUM_OF_CLASSESS])
        W_t4 = utils.weight_variable([16, 16, NUM_OF_CLASSESS, deconv_shape3[3].value], name="W_t4")
        b_t4 = utils.bias_variable([NUM_OF_CLASSESS], name="b_t4")
        conv_t4 = utils.conv2d_transpose_strided(fuse_3, W_t4, b_t4, output_shape=deconv_shape4, stride=8)
