...
def inference(image, keep_prob):
    ...
    mean_pixel = np.append(mean_pixel, [30.6986130799, 283.307,106.314329243,124.171918054,109.260369903,182.615729022,
...
def infer_little_img(input_image_path,patch_size=224,stride_ver=112,stride_hor=112):
    ...
    A_image = imread(input_image_path.replace('top','A').replace('_mosaic_09cm_area',''))
    A_image = np.expand_dims(A_image, axis=2)
    azi_image = imread(input_image_path.replace('top', 'azi').replace('_mosaic_09cm_area', ''))
    azi_image = np.expand_dims(azi_image, axis=2)
    B_image = imread(input_image_path.replace('top', 'B').replace('_mosaic_09cm_area', ''))
    B_image = np.expand_dims(B_image, axis=2)
    ele_image = imread(input_image_path.replace('top', 'ele').replace('_mosaic_09cm_area', ''))
    ele_image = np.expand_dims(ele_image, axis=2)
    entpy_image = imread(input_image_path.replace('top', 'entpy').replace('_mosaic_09cm_area', ''))
    entpy_image = np.expand_dims(entpy_image, axis=2)
    entpy2_image = imread(input_image_path.replace('top', 'entpy2').replace('_mosaic_09cm_area', ''))
    entpy2_image = np.expand_dims(entpy2_image, axis=2)
    L_image = imread(input_image_path.replace('top', 'L').replace('_mosaic_09cm_area', ''))
    L_image = np.expand_dims(L_image, axis=2)
    ndvi_image = imread(input_image_path.replace('top', 'ndvi').replace('_mosaic_09cm_area', ''))
    ndvi_image = np.expand_dims(ndvi_image, axis=2)
    sat_image = imread(input_image_path.replace('top', 'sat').replace('_mosaic_09cm_area', ''))
    sat_image = np.expand_dims(sat_image, axis=2)
    texton_image = imread(input_image_path.replace('top', 'texton').replace('_mosaic_09cm_area', ''))
    texton_image = np.expand_dims(texton_image, axis=2)

    height = np.shape(input_image)[0]
    width = np.shape(input_image)[1]
    output_image = np.zeros_like(input_image)
    input_image= np.concatenate((input_image,ndsm_image,dsm_image,A_image,azi_image,B_image,ele_image,
                                 entpy_image, entpy2_image, L_image, ndvi_image, sat_image, texton_image),axis=2)
    ...
    image = tf.placeholder(tf.float32, shape=[None, IMAGE_SIZE, IMAGE_SIZE, 15], name="input_image")
